...
/// On-device coach when no Gemini key is set — uses OpenAir scores only.
class LocalCoach {
  const LocalCoach();
  ...
  String answer({
    ...
    if (recentDays.isEmpty) {
      return 'No metrics loaded yet. Connect Google Health (or keep demo on) and pull to refresh, then ask again.';
    }
    ...
    if (q.contains('sleep')) {
      return 'Sleep performance ${sleep.performance.toStringAsFixed(0)}% '
          ...
          '${sleep.summary} '
          '(Local coach — add a Gemini key later for freer chat.)';
    }
    if (q.contains('hrv') || q.contains('heart') || q.contains('recovery')) {
      return 'Recovery ${(day.recoveryScore ?? 0).toStringAsFixed(0)} · '
          ...
          '(Local coach — Gemini optional later.)';
    }
    ...
    return 'Recovery ${(day.recoveryScore ?? 0).toStringAsFixed(0)} · '
        ...
        '$insight '
        '(Local OpenAir coach — paste a free Gemini key in Settings anytime for smarter chat.)';
  }
}
